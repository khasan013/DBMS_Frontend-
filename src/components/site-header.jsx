...
import { Box, Bus, LogIn, Menu, Moon, Plus, Search, Settings, Sun, UserPlus, UserRound, X } from "lucide-react";
...
export function SiteHeader() {
  ...
  const runSearch = (event) => { event.preventDefault(); const form = new FormData(event.currentTarget); const section = String(form.get("section")); if (section === "lost") navigate({ to: "/lost-and-found" }); else if (section === "market") navigate({ to: "/marketplace" }); else if (section === "shuttle") navigate({ to: "/shuttle" }); };
  const links = [{ to: "/lost-and-found", label: "Lost & Found" }, { to: "/marketplace", label: "Marketplace" }, { to: "/shuttle", label: "Shuttle Service" }];
  return <>
    <header className="sticky top-0 z-40 border-b border-border/80 bg-background/92 backdrop-blur-xl">
      ...
      {mobileOpen && <nav className="container-shell grid gap-1 border-t border-border py-3 lg:hidden">{links.map((link) => <Link key={link.to} to={link.to} onClick={() => setMobileOpen(false)} className="rounded-lg px-3 py-2 text-sm font-medium hover:bg-accent">{link.label}</Link>)}<Button className="mt-2 sm:hidden" onClick={() => requireLogin(() => setCreateOpen(true))}><Plus/>Create Post</Button></nav>}
