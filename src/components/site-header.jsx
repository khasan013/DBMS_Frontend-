...
import { LogIn, Menu, Plus, Search, Settings, ShieldCheck, UserPlus, UserRound, X } from "lucide-react";
...
export function SiteHeader() {
  ...
  const runSearch = (event) => { event.preventDefault(); const form = new FormData(event.currentTarget); const section = String(form.get("section")); if (section === "lost") navigate({ to: "/lost-and-found" }); else if (section === "market") navigate({ to: "/marketplace" }); else if (section === "to-let") navigate({ to: "/to-let" }); else if (section === "food") navigate({ to: "/food" }); };
  ...
  return <>
    <header className="sticky top-0 z-40 border-b border-border/80 bg-background/92 backdrop-blur-xl">
      <div className="container-shell flex h-16 items-center gap-5">
        ...
        <div className="ml-auto flex items-center gap-2">{!isAdmin && <Button className="hidden sm:inline-flex" onClick={() => requireLogin(() => setCreateOpen(true))}><Plus />Create Post</Button>}
