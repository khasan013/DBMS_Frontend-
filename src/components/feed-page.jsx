...
export function FeedPage({ module }) {
    ...
    () =>
      base.filter(
        (item) =>
          ...
          (!active.length ||
            active.some((filter) =>
              `${item.status} ${item.tag} ${item.meta}`
                .toLowerCase()
  ...
  const groups =
    module === "lost"
      ? [
          { title: "Type", options: ["Lost", "Found"] },
          {
            title: "Category",
            options: ["Electronics", "Bags", "Keys", "Documents"],
          },
          { title: "Status", options: ["Active", "Resolved"] },
        ]
      : [
          {
            title: "Category",
            options: ["Books", "Electronics", "Furniture", "Cycles"],
          },
          { title: "Condition", options: ["New", "Like new", "Used"] },
          { title: "Listing type", options: ["Sell", "Exchange", "Donate"] },
        ];
  const toggle = (value) =>
    setActive((old) =>
      old.includes(value)
        ...
        <div className="container-shell py-10 sm:py-14">
          <div className="flex max-w-2xl items-start gap-4">
            ...
            <div>
              ...
              <p className="mt-3 text-muted-foreground">{content.copy}</p>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {filtered.length} community posts
            </p>
          </div>
