...
import { Check, ChevronLeft, ChevronRight, ImagePlus, Search, ShoppingBag, X } from "lucide-react";
  ...
  return (
    <Dialog open={open} onOpenChange={openChanged}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
        ...
        {step === 1 && <div className="grid gap-3 sm:grid-cols-2">{options.map((option) => <button key={option.id} type="button" onClick={() => setType(option.id)} className={cn("rounded-xl border p-4 text-left transition", type === option.id ? "border-primary bg-primary-soft ring-2 ring-primary/15" : "border-border bg-card hover:bg-accent")}><option.icon className="mb-5 size-6 text-primary" /><strong className="block text-sm">{option.label}</strong><span className="mt-1 block text-xs leading-relaxed text-muted-foreground">{option.note}</span></button>)}</div>}
        {step === 2 && <div className="grid gap-5 sm:grid-cols-2">
          <div className="sm:col-span-2"><Label htmlFor="title">Title</Label><Input id="title" className="mt-2" placeholder={type === "lost" ? "e.g. Lost black earbuds" : "e.g. Scientific calculator"} /></div>
          <div><Label htmlFor="amount">{type === "market" ? "Price" : "Location"}</Label><Input id="amount" className="mt-2" placeholder={type === "lost" ? "Where was it lost or found?" : "৳ 0"} /></div>
          <div><Label htmlFor="category">{type === "market" ? "Condition" : "Item category"}</Label><select id="category" className="mt-2 h-9 w-full rounded-md border border-input bg-background px-3 text-sm"><option>Select an option</option><option>Like new</option><option>Used</option></select></div>
