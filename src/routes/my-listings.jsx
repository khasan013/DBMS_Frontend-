...
function MyListings() {
  ...
  return <main className="container-shell py-10"><section className="flex flex-col justify-between gap-5 border-b border-border pb-8 sm:flex-row sm:items-center"><div className="flex items-center gap-4"><Avatar className="size-16 text-xl font-bold"><AvatarImage src={user?.profileImgUrl} alt={`${user?.name || "Your"} profile`} /><AvatarFallback className="bg-primary text-primary-foreground">{initials}</AvatarFallback></Avatar><div><div className="flex flex-wrap items-center gap-2"><h1 className="font-display text-2xl font-bold">{user?.name || "My profile"}</h1><span className="flex items-center gap-1 rounded-full bg-success-soft px-2 py-1 text-xs font-semibold text-success"><ShieldCheck className="size-3.5" />Verified Student</span></div><p className="mt-1 text-sm text-muted-foreground">Your live Campus Crate posts</p></div></div><Button asChild><Link to="/edit-profile"><Edit3 />Edit profile</Link></Button></section>
