...
import { ArrowRight, Building2, Loader2, Search, ShieldCheck, ShoppingBag, Box, CircleCheck } from "lucide-react";
...
export const Route = createFileRoute("/")({
  head: () => ({ meta: [{ title: "Campus Crate — Your Campus, Connected" }, { name: "description", content: "Find lost items, student deals, and student-friendly rentals in one community." }, { property: "og:title", content: "Campus Crate — Your Campus, Connected" }, { property: "og:description", content: "Find lost items, student deals, and student-friendly rentals in one community." }, { property: "og:type", content: "website" }, { name: "twitter:card", content: "summary_large_image" }] }),
  component: Index,
});
...
function Index() {
  ...
  useEffect(() => {
    Promise.all([api("/api/items"), api("/api/marketplace/posts/active"), api("/api/to-let/listings")])
      .then(([items, marketplacePosts, toLetListings]) => setListings([
        ...items.map((item) => ({ id: `lost-${item.itemId}`, module: "lost", title: item.title, detail: `Location #${item.locationId}`, meta: item.createdAt ?? "Recently", tag: item.itemType, status: item.status, imageUrl: item.imageUrl })),
        ...marketplacePosts.map((post) => ({ id: `market-${post.postId}`, module: "market", title: post.title, detail: post.fixedPrice ? `৳${post.fixedPrice}` : `Starting ৳${post.startingPrice}`, meta: post.condition, tag: post.sellingType, status: post.status, owner: `Student #${post.sellerId}` })),
        ...toLetListings.map((listing) => ({ id: `to-let-${listing.listingId}`, module: "to-let", title: listing.title, detail: `৳${Number(listing.monthlyRent).toLocaleString()}/mo`, meta: `${listing.bedrooms} bed · ${listing.bathrooms} bath`, tag: listing.area, status: listing.status, owner: listing.contactPhone })),
      ]))
      .catch(() => setListings([]))
  ...
  return <main>
    ...
    <footer className="container-shell flex flex-col gap-4 py-8 text-sm text-muted-foreground sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-2 font-display font-bold text-foreground"><span className="flex size-7 items-center justify-center rounded-lg bg-primary text-primary-foreground"><Box className="size-4" /></span>Campus Crate</div><p>Made for campus life. Be kind, meet safely, and look out for each other.</p></footer>
  </main>;
}
